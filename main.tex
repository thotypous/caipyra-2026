...
\begin{frame}{Camada de transporte: TCP}
  \begin{columns}[T,onlytextwidth]
    \column{0.35\linewidth}
    \eyebrow{P2: a prática mais difícil}
    \begin{tightitemize}
      \item TCP é uma máquina de estados
      \item eventos mudam variáveis como \texttt{base}, janela e timer
      \item Go-Back-N é o modelo didático mais próximo do transmissor
    \end{tightitemize}

    \vspace{0.6em}
    \callout{\footnotesize
      A ideia central: confiabilidade aparece de ACKs, temporizadores e
      retransmissões, não de uma chamada mágica de socket.
    }

    \column{0.61\linewidth}
    \centering
    \includegraphics[width=\linewidth]{assets/kurose_gbn_fsm.pdf}

    \vspace{-0.2em}
    {\tiny\color{caipyraMuted} FSM do transmissor Go-Back-N, Kurose \& Ross}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{TCP em Python: eventos viram callbacks}
  \begin{columns}[T,onlytextwidth]
    \column{0.54\linewidth}
    \eyebrow{Chegada de segmento da camada de rede}
\begin{lstlisting}[style=py]
class Servidor:
...
\end{lstlisting}
    ...
    \eyebrow{Como o asyncio ajuda}
    \begin{tightitemize}
      \item a camada IP chama \texttt{\_rdt\_rcv} quando chega um segmento
      \item esse método é a transição \texttt{rdt\_rcv(rcvpkt)} da FSM
      \item o código não precisa ficar preso em um \texttt{while} esperando pacote
    \end{tightitemize}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{TCP em Python: timer e ACK atualizam estado}
  \begin{columns}[T,onlytextwidth]
    \column{0.47\linewidth}
    \eyebrow{Expiração de timer}
\begin{lstlisting}[style=py]
...
\end{lstlisting}
    ...
    \eyebrow{ACK cumulativo recebido}
\begin{lstlisting}[style=py]
...
\end{lstlisting}
  ...
  \vspace{0.25em}
  \callout{\footnotesize
    Com \texttt{asyncio}, eventos da FSM aparecem como chamadas de função:
    chegada de ACK e expiração de timer atualizam filas, janela e temporizador.
  }
\end{frame}

\begin{frame}{IPv4: destino final ou roteador?}
  \begin{columns}[T,onlytextwidth]
    \column{0.52\linewidth}
    \eyebrow{A primeira decisão}
    \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      >=Latex,
      ...
      arrow/.style={->, thick, draw=caipyraRed}
    ]
      \node[box] (pkt) {Datagrama IPv4 recebido};
      \node[decision, below=of pkt] (dec) {destino é meu IP?};
      \node[box, below left=0.55cm and 0.1cm of dec] (local) {entrega local\\TCP, UDP ou ICMP};
      \node[box, below right=0.55cm and 0.1cm of dec] (forward) {encaminha\\para outro salto};

      \draw[arrow] (pkt) -- (dec);
      \draw[arrow] (dec) -- node[left, font=\tiny, color=caipyraMuted] {sim} (local);
      \draw[arrow] (dec) -- node[right, font=\tiny, color=caipyraMuted] {não} (forward);
    \end{tikzpicture}
    }
    ...
    {\footnotesize
    \begin{tightitemize}
      \item o mesmo código pode receber pacotes destinados à própria máquina
      \item se o destino é local, o payload sobe para a camada de transporte
      \item se não é local, o sistema age como roteador
      \item só nesse segundo caso entra a tabela de rotas
    \end{tightitemize}
    }

    \vspace{0.25em}
    \callout{\scriptsize
      O slide seguinte mostra justamente essa parte de roteador:
      escolher o próximo salto por \textit{longest prefix match}.
    }
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Camada de rede: IPv4}
  \begin{columns}[T,onlytextwidth]
    \column{0.52\linewidth}
    \eyebrow{Escolher o próximo salto}
\begin{lstlisting}[style=py]
...
\end{lstlisting}
    ...
    \column{0.44\linewidth}
    \eyebrow{Longest prefix match}
    {\footnotesize
    \begin{tightitemize}
      \item roteador consulta uma tabela: prefixo $\rightarrow$ próximo salto
      \item \texttt{192.168.10.0/24} cobre todos os destinos com esses 24 bits iniciais
      \item \textit{longest prefix match}: entre rotas compatíveis, vence a mais específica
      \item isso permite uma rota geral e exceções mais detalhadas na mesma tabela
    \end{tightitemize}
    }

    \vspace{0.25em}
    \callout{\scriptsize
      No código, a máscara compara só o prefixo; \texttt{best\_match\_n}
      guarda a rota compatível mais específica.
    }
  \end{columns}
\end{frame}
...
\begin{frame}{O padrão que se repete}
  ...
  \begin{tikzpicture}[node distance=0.55cm, every node/.style={font=\small}]
    ...
    \node[align=left, right=1.0cm of tcp, text width=6.7cm] {
      \eyebrow{Interface de cada camada}
      \texttt{registrar\_recebedor(callback)}\\
      \texttt{enviar(payload, destino)}\\[0.4em]
      \footnotesize
      O callback explicita a direção \mbox{``subindo''}, e o método \texttt{enviar}
...
\begin{frame}{Por que Rust, se quase ninguém viu Rust?}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{tightitemize}
      \item BCC/EnC São Carlos desde 2018: zero planos com Rust
...
\begin{frame}{Viterbi: benchmark do gargalo}
  ...
  \vspace{0.15em}
  \callout{\scriptsize
    Na suíte completa: Viterbi em CPython puro, 55,0 s; em Numba, 35,0 s
    (36,6 s incluindo compilação inicial). Diferença observada: cerca de 20 s.
  }
\end{frame}
